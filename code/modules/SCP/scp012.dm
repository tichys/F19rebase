...
/obj/item/paper/scp012/Initialize()
	...
	SCP.memetic_sounds = list('sound/scp/scp012/012start.ogg','sound/scp/scp012/012mid.ogg') //not including the end file since if its playing that we can assume the person is being relieve of the effect
	SCP.compInit()
...
/obj/item/paper/scp012/proc/memetic_effect(mob/living/carbon/human/H)
	...
	if(get_dist(H, src) > 1)
		...
		if(prob(40) && H.getBruteLoss())
			...
			if(prob(50))
				H.visible_message(span_notice("[H] looks at \"[name]\" and sighs dejectedly."))
