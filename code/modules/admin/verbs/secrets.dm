	...
	switch(action)
		...
		if("maint_access_engiebrig")
			...
			for(var/obj/machinery/door/airlock/maintenance/M in INSTANCES_OF(/obj/machinery/door))
				M.check_access()
				if (ACCESS_ENGINEERING_LVL1 in M.req_access)
					M.req_access = list()
		...
		if("ancap")
			...
			if(SSeconomy.full_ancap)
				priority_announce("The NAP is now in full effect.")
			else
				priority_announce("The NAP has been revoked.")
		if("set_lights")
			if(!is_funmin)
				return
			var/state = tgui_alert(usr, "Do you want to turn all area lights On or Off?", "Set all lights", list("On", "Off"))
			if(state)
				SSblackbox.record_feedback("nested tally", "admin_secrets_fun_used", 1, list("Set All Lights", state))
				message_admins("[key_name_admin(holder)] turned all lights [state]")
				for(var/area/A in GLOB.sortedAreas)
					A.lightswitch = (state == "On" ? TRUE : FALSE)
					A.power_change()
