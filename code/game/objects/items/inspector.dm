...
/obj/item/inspector/attack_self(mob/user)
	. = ..()
	if(do_after(user, time = time_list[time_mode], progress=TRUE))
		print_report(user)
...
/obj/item/inspector/CtrlClick(mob/living/user)
	if(!user.canUseTopic(src, USE_CLOSE|USE_DEXTERITY) || !cell_cover_open || !cell)
		return ..()
...
/obj/item/paper/fake_report/water/AltClick(mob/living/user, obj/item/I)
	...
	else if(do_after(user, src, 1 SECONDS, progress=TRUE))
